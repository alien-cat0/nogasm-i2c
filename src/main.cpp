...
// Aug 2024 - Add I2C communication - aliencat
...
{
  if (motSpeed > 245)
    analogWrite(MOTPIN, 245); // make sure the frequency is audible
  else if (motSpeed < 10)
    analogWrite(MOTPIN, 10);
...
{
  if (myEnc.read() > maxVal * 4)
    myEnc.write(maxVal * 4);
  else if (myEnc.read() < minVal * 4)
    myEnc.write(minVal * 4);
...
void run_opt_speed()
{
  ...
  if (visRamp <= FREQUENCY * NUM_LEDS - 1)
    visRamp += 16;
  else
    visRamp = 0;
...
void sendingToBridge()
{
  Wire.beginTransmission(I2C_ADDRESS);

  // Send motor speed (1 byte)
  // If speed is negative, send 0
  uint8_t speedToSend = (motSpeed < 0) ? 0 : (uint8_t)motSpeed;
  Wire.write(speedToSend);

  // Send pressure (2 bytes, big-endian)
  Wire.write((pressure >> 8) & 0xFF); // High byte
  Wire.write(pressure & 0xFF);        // Low byte

  // Send avgPressure (2 bytes, big-endian)
  Wire.write((avgPressure >> 8) & 0xFF); // High byte
  Wire.write(avgPressure & 0xFF);        // Low byte

  Wire.endTransmission();
}

void requestFromBridge()
{
  Wire.requestFrom(I2C_ADDRESS, 1); // Request 1 byte from slave
  if (Wire.available())
  {
    int newSpeed = Wire.read(); // Read the new speed from slave
